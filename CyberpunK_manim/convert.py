...
from manim_slides import templates
from manim_slides.commons import folder_path_option, verbosity_option
...
class Converter(BaseModel):  # type: ignore
    ...
    def convert_to(self, dest: Path, theme: Path) -> None:
...
class RevealJS(Converter):
    ...
    def convert_to(self, dest: Path, theme: Path) -> None:  # noqa: C901
        """
...
class HtmlZip(RevealJS):
    ...
    def convert_to(self, dest: Path, theme: Path) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            empty_theme_layout = Path()

            html_file = directory / dest.with_suffix(".html").name

            super().convert_to(html_file, empty_theme_layout)
...
@click.option(
    ...
    help="Use the layout given by FILE instead of default one. ",
...
def convert(
    ...
    try:
        ...
        converter.convert_to(dest, theme_layout)
